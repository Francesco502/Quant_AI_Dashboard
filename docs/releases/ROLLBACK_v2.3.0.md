# Rollback Guide `v2.3.0`

## When To Roll Back

- A `v2.3.0` deployment introduces environment-specific regressions.
...
- `v2.3.0` is intended to remain backward-compatible with the current SQLite-based runtime layout.
- If you change LLM timeout/retry variables during upgrade, keep the pre-upgrade `.env` backup so rollback can restore both code and runtime configuration together.
