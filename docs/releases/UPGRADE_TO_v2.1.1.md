...
- CI workflow switched to manual trigger (`workflow_dispatch`) only.
- CI execution paths hardened to reduce `exit code 4` startup failures.
- Deployment and runtime metadata updated to `v2.1.1`.

## Server Upgrade From `2.0.0` To `2.1.1`
...
- Upgrade from `2.0.0` to `2.1.1` does not require destructive SQLite migration.
- Existing user assets and trading data are preserved under normal compose-based upgrade.
