...
- The release line standardizes on the current `2.2.0` frontend and backend runtime model.
- Release validation is now centered on `python scripts/release_check.py`.
- Current operating documentation is consolidated under `docs/current/`.
...
- optional provider keys such as `TUSHARE_TOKEN`, `OPENAI_API_KEY`, `ARK_API_KEY`
...
- `v2.2.0` is intended to preserve the current SQLite/runtime layout under normal compose-based upgrades.
